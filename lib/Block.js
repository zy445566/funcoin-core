...
class Block{
    ...
    async acceptBlock(block)
    {
        ...
        let inBlock = this.getBlock(block.id,block.result,block.producerAddress,parentBlock)
        let key = this.getBlockKey(
            inBlock.id,
            inBlock.timestamp
        );
    ...
    {
        let block = this.getBlock(id,null,this.wallet.getWalletAddress());
        let key = this.getBlockKey(
            block.id,
            block.timestamp
        );
    ...
    getBlock(id,result,producerAddress,parentBlock={})
    {   
        ...
        if (config.totalNum<(parentBlock.hight+1)*config.awardNum)
        {
            return false;
        }
